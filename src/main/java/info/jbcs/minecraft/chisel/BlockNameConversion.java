...
public class BlockNameConversion
{
            ...
            switch (step) {
                ...
                case 2: // rename "marbleSlab" -> "marble_slab", "blockRedstone" -> "redstone_block"
                    ...
                    if (newname.startsWith("block_"))
                    {
                        ...
                        FMLLog.getLogger().warn("   Renaming1 \"block_\" to " + newname);
                    }
                    else if (newname.startsWith("snakestone_"))
                    {
                        newname = newname.replaceFirst("snakestone_([a-z_]+)", "$1_snakestone");
                        FMLLog.getLogger().warn("   Renaming2 \"block_\" to " + newname);
                    }
                    return newname;
                case 3: // rename "wood-dark-oak"/"wood.dark-oak" -> "dark_oak_planks"
                    newname = oldname.replaceAll("([a-z]+)([A-Z])", "$1_$2").toLowerCase().replace('-', '_');
                    if (newname.contains("wood"))
                    {
                        newname = newname.replaceFirst("wood[_\\.]([a-z_]+)", "$1_planks");
                    }
                    return newname;
    ...
    static public void init()
    {
        ...
        // Names used in 1.6 (in both AUTOMATIC_MAIDEN and Delta534)
        namingConversion.put("snakestone", "stone_snakestone");
        namingConversion.put("ItemChisel", "chisel");
        namingConversion.put("thinGlass", "glass_pane");
    }
